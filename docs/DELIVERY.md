...
| 8 | 射程几何 | ⏳ 有 `rangeId` 无形状 → **群攻召唤物实际总输出高于对单值**；范围类特性同理 |
| 9 | 召唤物技能的目标数 / 自身防御生命强化 | ⏳ 引擎支持 `units.hits`（显式传），默认不叠加 |
| 10 | 集火模型的地形分流与阻挡分散 | ⏳ 多个敌人被不同阻挡位分摊时，结果会好于本栏 |
| 11 | 模组 `※` 叠加备注 | PRTS 有（如"多个伏击客X模组间减速可叠加"），未采集 |
| 12 | 无法判定"召唤物是否会普攻" | 数据层区分不出"有普攻的召唤物"与"只靠触发的装置" → 报告并列两条线并标注，**不猜** |
