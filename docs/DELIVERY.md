...
| **召唤物专项** | `node tools/verify-summons.mjs` | **51** | 分类判据 / 数值手工验算 / **通道不双算** / 不渗入本体 / 自身技能倍率 / 触发伤害 |
| **条件型专项** | `node tools/verify-conditional.mjs` | **23** | 对空加成 / 索敌来源含特性 / 蓄力两态，且**不污染锚点** |
...
| 5 | 召唤物自身生存 | ⏳ 未进 ④ 生存栏（Mon3tr 5433 血/阻挡3 不算进本体承伤）|
| 6 | 召唤物技能的范围/目标数 | ⏳ 未建模；弦惊"高级形态"真实加成不在数据层 |
| 7 | 无法判定"召唤物是否会普攻" | ⏳ 数据层区分不出"有普攻的召唤物"与"只靠触发的装置" → 报告并列两条线并标注，**不猜** |
| 8 | 模组条件型特性折算 | 对空/距离类只标注不计入（阻挡类已按"持续阻挡假设"计入）|
| 9 | 模组 `※` 叠加备注 | PRTS 有（如"多个伏击客X模组间减速可叠加"），未采集 |
| 10 | 生存多敌人集火模型 | 现按单敌人给数，集火靠人工判断 |
| 11 | 基础特性的范围/索敌几何 | 技能 `rangeId` 未入库，范围形状未建模 |
