...
│   ├── summon.mjs              召唤物接线层（data → 引擎 `summon` 字段 + 报告行 + 召唤物生存/空间）
│   ├── conditional.mjs         条件型加成与索敌层（对空/优先攻击/蓄力两态，只标注不进 DPS）
│   ├── range.mjs               射程几何层（形状 + 格数/射程/盲区 + ASCII 形状图）
│   ├── build-ranges.mjs        range_table → data/ranges.json
...
│   ├── summons.json            74 召唤物（引擎可见 73：**伤害型 41 / 功能型 8** / 装置 24）
│   ├── summon-skills.json      95 个召唤物自身技能（§18 伤害改量 / §19 触发伤害 / §23 范围）
│   ├── ranges.json             73 个射程几何（形状 + tiles/reach/nearest，§24）
...
### 3.3 栏位文本的"双形态"（2026 新增，修正了"把 agent 旁白糊到用户脸上"）

各栏的 `formatXxxSection()` 产出的是**同一份字符串**，同时喂给终端报告（给 agent）与网页（给用户）。
于是用户界面上出现了**不是给用户看的**内容：标题里塞的元信息
（`【泛用性】（关键点之一：换场景还强不强；可用线 = 全库 433 名干员分布）`）与结尾的口径旁白
（`说明：…需 agent 判断`）。

**解法**（`tools/section.mjs` 的 `splitSections`）：把一份文本切成两半 ——
- `content` = **展示形态**（标题剥掉括号元信息、结尾旁白剥离）→ **网页渲染这个**
- `notes` = 口径旁白 → **只给 agent / 终端用**
- `result.*Section` **原文一字不动** → LLM 与终端报告**零信息损失**

API 的 `POST /api/evaluate` 因此同时返回 `sections`（网页用）与 `result`（agent 用）。
**注意这不是"数据给谁看"的区别，而是"同一份内容的两副面孔"**：数值来源完全相同。
...
| **召唤物 DPS** | Mon3tr **701.0** · 令弦惊 **548.7** · 打字机 **3450.5**（含自身技能 ×6.375）· 傀影虚影 **707.1** · 衡沙发条羽兽 **340.0** · 魂灵之影 777.0（「最多存在3个」→ 上限 2331）· 小自在 209.5 · 蓄水炮 243.8（**独立输出线，不与本体相加**）|
| **通道互斥** | 罗宾/霜华/多萝西/钼铅/望 **不在** summon 通道（归 trap/burst，防双算） |
| **召唤物自身技能** | 42 条伤害型里只有 **3 条**需改量（其余属一次性入场/周期/概率型，不计入持续 DPS）|
| **召唤物触发伤害** | 梅尔「爆破回收」每次 **2664** · 温蒂「液氮大炮」2048 · 鸿雪「点题」1992 · 傀影「夜幕突袭」1644（**按次结算，不给 DPS**）|
| **对空加成** | 阿罗玛 S1 专三 **90%**（全库唯一；默认地面场景**不计**，仅给空战参考 ×1.90）|
| **索敌规则** | **57 名干员**（其中 23 名狙击的"优先攻击空中单位"来自**特性** → 模式来源必须含特性）|
| **蓄力两态** | 9 名（卡涅利安 atk 0.45→0.6 · 灵知 1.7→2 · 龙舌兰 2.1→2.3 · 斥罪 1.9→2），两态并列**不自动改基准** |
| **模组条件型** | 按条件类型分类（air 10 / distance 4 / unblocked 4 / blocked 16），**不计入基准**：能天使 1994.6→2194.0（对空×1.1）· 帕拉斯 1057.1→1374.3（未阻挡×1.3）· 伊芙利特 457.1→502.9（距离×1.1）|
| **生存集火** | `surviveGroup`：同时命中 = perHit×N 保持间隔。1758血/356防 vs 550攻/3s：1 个 10 击/30s → **×2 = 3 击/9s**；泥岩 ×2 狂暴 2 击/2.6s |
| **召唤物生存** | Mon3tr 5433/405/阻挡3 → 物理·精英 每击 595 · 10 击 · 26s；温蒂蓄水炮 100 血 → **1 击倒**（单列，不与本体合并）|
| **射程几何** | 艾雅法拉 S3 **25 格菱形** · 银灰 S3 16 格 · 伊芙利特 6 格直线（射程 5）· 史尔特尔 2 格；面板 454/454、技能改射程 237/994 |
...
| **"射程几何未建模"** | 我据此写进了文档与总结，并说"只有 id 没有形状" | **错**：`range_table.json` 一直在数据仓库里（73 个范围带完整 `{row,col}` 坐标）。真缺口是 `rangeId` 在 `phases[n].rangeId`（phase 级），而 `maxPanel()` 只读 `attributesKeyFrames.data` 就把它丢了 | 用户追问"射程数据库或 prts 里面没有吗"后核实 |
...
| # | 项 | 状态 |
|---|---|---|
| 1 | **召唤物通道** | ✅ **已完成**（`summon` 通道 + `tools/verify-summons.mjs` 51 断言；Mon3tr 701.0 / 令弦惊 548.7 / 打字机 3450.5） |
| 2 | **召唤物自身技能** | ✅ **已完成**（§18：42 条里只有 3 条需改量，其余属一次性/周期/概率型） |
| 3 | **召唤物触发伤害** | ✅ **已完成**（§19：burst 层只报"每次触发"、不给 DPS；梅尔 2664 / 温蒂 2048） |
| 4 | **条件型加成与索敌** | ✅ **已完成**（§20：对空加成 / 57 名索敌 / 9 名蓄力两态） |
| 5 | **模组条件型特性折算** | ✅ **已完成**（§21：按条件类型分类 air/distance/blocked/unblocked，不计入基准；能天使对空 ×1.1、帕拉斯 ×1.3） |
| 6 | **生存多敌人集火** | ✅ **已完成**（§22：`surviveGroup` 同时命中 = perHit×N，报告给 ×2/×3 档） |
| 7 | **召唤物自身生存 + 技能空间** | ✅ **已完成**（§23：召唤物生存单列；`rangeId` 15/42 条入库） |
| 8 | **射程几何** | ✅ **已完成**（§24：`range_table` 73 个范围接入；**更正：数据一直都在，此前是管线丢字段**） |
| 9 | 射程未并进 DPS | ⏳ 空间量，报告口径是对单目标；阻挡分流、地形限制、敌人走出范围的动态未建模 |
| 10 | 召唤物技能的目标数 / 自身防御生命强化 | ⏳ 引擎支持 `units.hits`（显式传），默认不叠加 |
| 11 | 集火模型的地形分流与阻挡分散 | ⏳ 多个敌人被不同阻挡位分摊时，结果会好于本栏 |
| 12 | 模组 `※` 叠加备注 | PRTS 有（如"多个伏击客X模组间减速可叠加"），未采集 |
| 13 | 无法判定"召唤物是否会普攻" | 数据层区分不出"有普攻的召唤物"与"只靠触发的装置" → 报告并列两条线并标注，**不猜** |
