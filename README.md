...
数值来自游戏客户端数据（[Kengxxiao/ArknightsGameData](https://github.com/Kengxxiao/ArknightsGameData) 的 zh_CN 数据表），
核心公式与机制表与 [PRTS](https://prts.wiki) 核对过。

以下是**当前没有计算**的部分，评估时需自行考虑：

- **极限轴** —— 何时开技能、如何排轴属玩法层，需自行给出参数（已支持传入）
- **队友支援** —— 生存栏按无人治疗计算，实际有医疗覆盖时结论会不同
- **地形** —— 阻挡分流、多路压力等未建模，需结合具体关卡判断
- **敌方伤害类型** —— 游戏数据中不包含（由 prefab 决定），来袭画像选取可确定判定类型的真实敌人作为锚点
- **潜能与信赖** —— 统一按专三满级面板计算，不含信赖加成
